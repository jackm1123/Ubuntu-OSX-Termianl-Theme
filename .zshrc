...
# My shell layout preferences
setopt PROMPT_SUBST
PROMPT='%F{green}%n@%F{green}%m%f:%F{blue}%~%f%B%F{cyan}${vcs_info_msg_0_}%b%f% $ %f'
# F{color} beforehand formats the color. n is name, m is machine, f removed color formatting for next thing, ~ is directory
...
export PATH="/usr/local/sbin:$PATH" # Fix path for brew warnings
...
alias cd..='cd ....'
alias cd..='cd .....'
alias proj='cd /Users/jack/Documents/projects'
alias gits='git status'
alias gitl='git log'

# save space, delete "other" space
remove_snapshots() {
	cd
